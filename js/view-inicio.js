...
(function (App) {
  ...
  function moduleCard(id, icon, title, desc){
    return '<button class="module-card" data-goto="'+id+'" type="button">'+
      '<span class="module-card__icon" aria-hidden="true">'+icon+'</span>'+
  ...
  App.views = App.views || {};
  App.views.inicio = {
    title: 'Resumen', desc: 'Panorama general del torneo y accesos directos.', icon: '🏠',
    render: function (el) {
      ...
      el.innerHTML =
        '<section class="hero-lg fade-in">'+
          ...
          '</div>'+
          '<div class="hero-lg__badge" aria-hidden="true">⚽</div>'+
        '</section>'+
        ...
        '<div class="module-grid fade-in">'+
          moduleCard('sedes','🏟️','Tour Virtual de Sedes','16 sedes; hacé clic para saltar a sus partidos.')+
          moduleCard('agenda','🗓️','Agenda Simultánea','Días con varios partidos, en columnas paralelas.')+
          moduleCard('timeline','♾️','Timeline Infinito','Los 104 partidos con carga progresiva al hacer scroll.')+
          moduleCard('fanatico','⭐','Dashboard del Fanático','Seguí a tu selección y tematizá el panel.')+
          moduleCard('matriz','🔲','Matriz de Enfrentamientos','Cuadrícula 4×4 de resultados por grupo.')+
        '</div>';
    }
  };
